...
function Button({label, disable, type, border, width, onClick, href}: ButtonProps) {
  
  if(href){
    return <Link href={href} className={`${css.btn} ${type === 'primary' ? css.btnPrimary : css.btnSecondary} ${border === 'full' ? css.regular : css.half} ${width && css.width}`} onClick={onClick}>{label}</Link>
  }
